...
- **Las consolas clásicas de Windows ahora muestran color en lugar de secuencias de escape crudas** — la paleta emitía códigos SGR en cualquier TTY de Windows, pero el host de consola heredado (cmd.exe, Windows PowerShell 5.1, binarios abiertos con doble clic) no los interpreta hasta que el proceso activa el procesamiento VT. `internal/cliui` ahora activa `ENABLE_VIRTUAL_TERMINAL_PROCESSING` en stdout en el primer uso y condiciona toda la salida ANSI a ello, de modo que `panda ask` / `doctor` / el REPL clásico imprimen texto plano limpio donde VT no puede activarse, en lugar de basura tipo `[31m`.
- **El REPL clásico de Windows obtiene un editor de línea real** — antes era una lectura plana con scanner: sin movimiento del cursor, sin historial, sin completado con Tab, y Ctrl-C durante una pregunta en curso golpeaba el manejador de control de consola y mataba todo el REPL. Un nuevo editor basado en la API de consola añade edición en línea, historial persistente (ahora en `%LOCALAPPDATA%\openpanda`), completado de comandos con barra y de posiciones de argumento, cancelación de la pregunta en curso con Esc/Ctrl-C (doble pulsación para salir, como en unix) y ancho de terminal real.
- **`panda uninstall` ahora limpia la configuración por defecto y la tarjeta de capacidades** — una configuración en la ubicación por defecto del usuario (p. ej. `%APPDATA%\openpanda\config.yaml`) se juzgaba como «custom location — kept», dejando la clave de API y el secreto compartido en disco; la tarjeta de capacidades nunca entraba en el plan. Ambas se consideran ahora propias cuando viven en los directorios de configuración estándar de usuario o del sistema (también corrige `~/.config/openpanda` en Linux).
- **Abrir el navegador en Windows ya no se rompe con caracteres especiales en la URL** — `cmd /c start <url>` interpreta `&`, `^` y los paréntesis como sintaxis de shell; `panda web` ahora abre la URL con token mediante `rundll32 url.dll,FileProtocolHandler`.
- **Endurecimiento de instalación/desinstalación en Windows** — la autocomprobación del instalador ahora falla con un código de salida distinto de cero en lugar de reportar éxito con un binario roto; la comparación de PATH expande las entradas `%VAR%` para no duplicarlas; la cancelación vía `taskkill` ya no muestra una ventana de consola; las sondas de hardware prefieren `pwsh` con respaldo en `powershell`; `SystemConfigDir` cae a `C:\ProgramData` si `ProgramData` no está definido; el historial del REPL pasa de `~/.local/state` a `%LOCALAPPDATA%\openpanda`.

## [0.0.8] - 2026-09-20

Lanzamiento oficial v0.0.8: OpenPanda pasa de la versión preview a la línea base estable oficial, introduciendo políticas de idioma en prompts y localización integral de extremo a extremo, clasificación geográfica de proveedores, sanitización automática de credenciales, optimizaciones de concurrencia SQLite WAL, refuerzo de los niveles mínimos de capacidad y correcciones en instaladores e interacción en terminal.

### Añadido

- **Política de idioma en prompts y localización desacoplada de salida** — `internal/core` implementa `PromptLanguagePolicy`, inyectando los requisitos de idioma del usuario en los prompts de los agentes y de ejecución con propagación de contexto, manteniendo limpias las instrucciones originales (b4ef8a6).
- **Flujo del motor localizado y selector de idioma en CLI** — los resúmenes de dispositivos, triaje rápido, envío de tareas, notas de progreso de subagentes y resúmenes de resultados ahora son compatibles con chino, inglés, japonés, español y alemán; accesibles mediante `--lang` en `panda ask`, REPL y Bubble Tea TUI (6ac309d, fc96d8c, 8d6ce28).
- **Clasificación geográfica de proveedores** — detección de origen de proveedores e inferencia `DetectRegion` en `internal/providers` para enrutar tareas dinámicamente según la región del host y los requisitos de cumplimiento (35443bc).
- **Sanitización automática de credenciales** — `internal/core/sanitize.go` escanea y redacta automáticamente claves API, claves privadas y tokens de autorización en el empaquetado de proyectos, cargas útiles de tareas y registros de ejecución (d91a4bf).

### Corregido

- **Concurrencia SQLite WAL y ajuste de bloqueos** — se optimizan los pragmas de conexión SQLite (`journal_mode=WAL`, `synchronous=NORMAL`, `busy_timeout=5000`) para eliminar la inanición de escritores bajo un tráfico elevado de nodos concurrentes (d91a4bf).
- **Refuerzo del límite mínimo de permisos** — se refuerza la evaluación de permisos defensivos para que las operaciones de alto impacto o irreversibles requieran estrictamente confirmación manual (d91a4bf).
...
- **TUI: un desplazamiento rápido ya no escribe coordenadas del ratón en la barra de entrada** — un gesto del trackpad son decenas de muescas de rueda, que desbordan el búfer de entrada de 256 bytes de Bubble Tea y cortan el evento por la mitad; las coordenadas X10 no llevan prefijo de escape propio, así que una vez consumido el prefijo los bytes restantes son indistinguibles de lo que se teclea y `[-` y `13M` acababan en la barra de entrada. El prefijo truncado sí se reporta, y ahora indica cuántos bytes debe todavía el evento: esos son los que se descartan. El guardián mira la forma del mensaje y nunca su contenido (el lector rellena ese búfer en cuanto se entrega el mensaje), y solo se arma mientras la aplicación retiene el ratón, por lo que no puede comerse una pulsación en el modo select por defecto.

## [0.0.8-preview] - 2026-09-19

### Añadido

- **Detección automática de tarjetas de capacidad y configuración en un paso** — el sondeo del host (hardware, CLI de agentes instaladas, clase de recursos) vive ahora en un paquete compartido `internal/carddetect` usado por `panda detect`, `panda card rescan/edit`, el onboarding y el panel web; un nodo sin tarjeta recibe una ensamblada automáticamente en lugar de fallar en el primer uso.
- **Inicialización diferida del planificador** — el motor de Ask ya no requiere `CardPath` por adelantado: deriva la ubicación de la tarjeta desde la configuración o la ruta por defecto y levanta el planificador en el primer despacho de tarea o mutación de tarjeta, de modo que las respuestas y las herramientas de memoria funcionan antes de que exista tarjeta alguna.
- **Ediciones estructuradas de tarjetas desde cualquier superficie** — las entradas de agente ganan actualizaciones por campo `card_agent_set`, y el panel web, el REPL/TUI y la CLI mutan `capabilities.yaml` a través de una única ruta compartida y validada (`internal/cardmut`) que conserva intactos los comentarios escritos a mano.
...
- **Un registro multimodelo para `/model`** — el modelo de entrada ya no es una única ranura. Una lista `models:` contiene modelos con nombre y `/model <alias>` alterna entre ellos, mientras que `/model add`, `/model list`, `/model fetch` y `/model test` dan soporte a DeepSeek, Claude, ChatGPT, Kimi, Volcengine Ark, Zhipu, Qwen, SiliconFlow, OpenRouter y Ollama (08ede13). Incluye normalización inteligente de `base_url` (barra final, prefijo de versión ausente, peculiaridades por proveedor), campo `reasoning` para modelos en modo pensamiento y presets de proveedor OpenAI.
...
- **Sandbox consciente de proxy** — el sandbox de ejecución ahora propaga las variables de entorno de proxy estándar (en mayúsculas y minúsculas) a los procesos hijos, de modo que las instalaciones y descargas enrutadas funcionan detrás de un proxy.
- **Descubrimiento de agentes más amplio** — el registro reconoce más alias de binarios (`claude-code`, `grok-build`, `deepseek-harness`, `hermes-agent`) y sondea los directorios de instalación habituales (`~/.local/bin`, `/opt/homebrew/bin`, …) además del `PATH`; el descubrimiento de credenciales de OpenCode sigue su disposición de configuración actual.
...
- **Comando `panda read`** — visualiza documentos markdown y texto directamente en el terminal con resaltado de sintaxis y preservación de tuberías.

### Mejorado

- **El TUI devuelve el ratón al terminal** — el front-end a pantalla completa ya no captura el ratón por defecto, así que la selección por arrastre, el doble clic y el atajo de copia del propio terminal vuelven a funcionar. El desplazamiento sobrevive sin él: el scroll alternativo convierte una muesca de rueda en una tecla de flecha, PageUp/PageDown siempre funcionan y `ctrl+t` alterna en vivo al antiguo modo de captura (`ui.mouse: scroll` / `PANDA_MOUSE=scroll` lo hacen permanente). Conviene saberlo en *cualquiera* de los dos modos: la captura nunca costó la copia del todo, solo la copia sin modificador — iTerm2 sigue seleccionando mientras mantienes `Option` (y copia al soltar), Terminal.app mientras mantienes `Fn`.
...
- **Prompt de sistema reescrito en inglés con un enrutado de tareas más firme** — ahora se exige explícitamente al modelo de entrada que responda a peticiones de ejecución o despacho de agentes emitiendo el JSON de tarea de inmediato, en lugar de conversar pasivamente; las palabras gatillo relacionadas con el despacho (`调度`, `派发`, `dispatch`, `schedule`, …) se reconocen en todos los idiomas que usa el prompt.
- **Los comandos slash y de shell se quedan dentro del TUI** — las líneas `/…` y `!…` ya no liberan el terminal: su salida se captura directamente en el scrollback, y la vista de chat gana desplazamiento con PgUp/PgDn y rueda del ratón para revisitarla.
- **Manejo de fallos de proveedor más rápido y preciso** — los adaptadores informan fallos de autenticación, cuota, 5xx y de proveedor con códigos de salida deterministas, el comandante normaliza nombres de agente y pares endpoint/modelo por adaptador (incluidos los endpoints duales de DeepSeek), y el enrutado recurre a un agente de propósito general cuando una tarea no nombra ningún requisito concreto, de modo que las inyecciones fallidas se recuperan en lugar de reintentar un endpoint muerto.
...
- **Consola: rediseño visual «Panda Paper» y pulido del flujo de trabajo** — un tema claro de papel cálido y un tema oscuro de tinta cálida (ni negro puro ni gris frío), acentos verde bambú atenuados, azul reservado para la ruta de decisión, tipografía serif solo en los encabezados, radios un paso mayores, sombras marrón cálido más suaves y eliminación de adornos «de IA» (texto degradado en títulos, subrayados degradados, capas de brillo en botones). Las vistas de sesión y de proyecto también se rehicieron conforme a la arquitectura del flujo de trabajo, con acciones y estados coherentes (291dc76). La capa actúa por redefinición de tokens y cascada: sin cambios en estructura, nombres de clase ni lógica de los componentes.

### Corregido

- **TUI: fuga de residuos de secuencias de escape en la entrada del terminal** — las secuencias ANSI cortadas o no analizadas (como paquetes de seguimiento de ratón SGR, informes de posición del cursor, teclas Kitty, prefijos Alt-corchete) divididas entre límites de lectura o generadas por movimiento de ratón no capturado ahora se filtran estrictamente y nunca se filtran como caracteres inválidos en la barra de entrada.
...
- **Resolución de configuración sin archivo de configuración** — la resolución de rutas recurre al directorio del usuario en lugar de `/etc`, y las escrituras de configuración crean antes los directorios padre; `node.card_path` se respeta como ubicación de la tarjeta.
- **Las ediciones de tarjeta ya no se convierten silenciosamente en no-ops** — añadir un id existente o eliminar uno inexistente es un error en todas partes, y el motor recarga la tarjeta tras cada mutación para que los cambios surtan efecto de inmediato.
- **La rueda sigue funcionando tras `ctrl+t` y durante `exec`** — volver al modo de selección ahora vuelve a solicitar el scroll alternativo (Bubble Tea nunca emite DECSET 1007 por sí mismo, así que el flag debe seguir al modo a mano), y los eventos de rueda se gestionan antes de la puerta de modo, de modo que releer una tarea de larga duración ya no requiere PageUp. Los clics siguen ignorándose mientras corre exec.
...
- **Se elimina la herramienta de gestión `taskq_create`** — la creación de tareas es ahora competencia exclusiva del modelo de entrada (salida de JSON de tarea, ejecutada de inmediato por el planificador) o llega desde el tablero o herramientas externas; las herramientas de cola (`taskq_list/show/cancel/priority/move`) permanecen para gestionar el backlog. Los agentes programados para llamar a `taskq_create` deben emitir una tarea o usar el tablero.
