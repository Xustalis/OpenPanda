...
`make web` builds `app/` into `panel/dist/`, where `go:embed` folds it into
every binary that links the panel package:
...
A committed placeholder in `panel/dist/` keeps `go build` working without
node. Ship UI changes only after a real `make web` build.
