...
- **TUI：快速滚动不再把鼠标坐标敲进输入框** —— 一次触控板滑动就是几十个滚轮事件，足以撑爆 Bubble Tea 的 256 字节读取缓冲并在事件中间断开；X10 的坐标本身不带转义前缀，前缀一旦被吃掉，剩下的字节与手打输入就无从区分，于是 `[-`、`13M` 这类内容直接落在输入框里。被截断的前缀仍会上报，现在由它算出该事件还欠多少字节，只把这些字节吞掉。该守卫只看消息的形状、绝不读其内容（读取缓冲在消息交出的瞬间就会被下一轮读取覆盖），且仅在应用持有鼠标时才武装，因此不会在默认的 select 模式下吃掉用户按键。

## [0.0.8-preview] - 2026-09-19

向正式开源项目转变的里程碑版本：OpenPanda 从实验性任务路由内核全面蜕变为正式、生产可用的分布式个人智能体编排操作系统。本综合预览版引入自带离线精选目录的技能中心（Skills Hub）、作为一等公民的项目与上下文感知委派、能力卡自动探测与多前端结构化编辑、支持运行中转向与终端原生鼠标选择的交互式全屏 TUI、Agent 主动故障转移与凭证兜底模型注入、多模型注册中心、执行全流程透明化跟踪，以及 Web 控制台工作流打磨。

### 新增

- **能力卡自动探测与一步建卡** —— 主机探测（硬件、已安装的 agent CLI、资源等级）统一到新共享包 `internal/carddetect`，`panda detect`、`panda card rescan/edit`、首次运行向导和 Web 面板共用同一份实现；没有能力卡的节点会在首次使用时自动组装建卡，而不是直接报错。
- **调度器懒初始化** —— ask 引擎不再要求启动时传入 `CardPath`：它从配置或默认路径推导卡片位置，并在首次任务派发或卡片变更时才拉起调度器，因此建卡之前问答和记忆工具即可使用。
- **各前端统一的卡片结构化编辑** —— agent 条目新增字段级 `card_agent_set` 更新；Web 面板、REPL/TUI 和 CLI 全部经由同一条校验过的变更路径（`internal/cardmut`）修改 `capabilities.yaml`，用户手写的注释保持不变。
...
- **多模型注册中心（`/model`）** —— 模型配置不再是单一字段。支持通过 `models:` 列表管理多个模型，并在运行期通过 `/model <alias>` 无缝热切换；内置支持 DeepSeek、Claude (Anthropic)、ChatGPT (OpenAI)、Kimi (月之暗面)、火山引擎 (Ark/豆包)、智谱、通义千问、硅基流动、OpenRouter 及本地 Ollama（08ede13）。
...
- **沙箱代理感知** —— 执行沙箱现在向子进程透传标准代理环境变量（大小写各一套），代理网络下的安装与下载可以正常工作。
- **更广的 agent 发现** —— 注册表识别更多二进制别名（`claude-code`、`grok-build`、`deepseek-harness`、`hermes-agent`），除 `PATH` 外还探测常用安装目录（`~/.local/bin`、`/opt/homebrew/bin` 等）；OpenCode 凭据发现跟随其现行配置布局。
...
- **`panda read` 命令** —— 终端内直接查看 Markdown 文档与文本，支持语法高亮与管道无损输出。

### 变更

- **TUI 把鼠标交还终端** —— 全屏界面默认不再捕获鼠标，拖选、双击选词与终端自带的复制快捷键恢复正常。滚动不依赖鼠标也不会丢：alternate scroll 把滚轮变成方向键、PageUp/PageDown 始终可用，`ctrl+t` 还能随时切回原捕获模式（`ui.mouse: scroll` / `PANDA_MOUSE=scroll` 可设为常驻）。两种模式下都值得知道的一点：捕获夺走的从来只是"不带修饰键的复制"——iTerm2 按住 `Option` 拖选、Terminal.app 按住 `Fn` 拖选，依旧可以选中（iTerm2 松手即入剪贴板）。
...
- **系统提示词英文化，任务路由更强硬** —— 入口模型现在被明确要求对执行类或 agent 调度类请求立即输出 task JSON，而不是被动对话；调度类触发词（"调度""派发""dispatch""schedule" 等）在所有提示词语言中都被识别。
- **斜杠命令和 shell 逃逸不再跳出 TUI** —— `/…` 和 `!…` 行不再释放终端：输出被直接捕获进回滚区，聊天视图新增 PgUp/PgDn 与鼠标滚轮滚动，方便回看。
- **更快更准确的 provider 故障处理** —— 适配器以确定性退出码上报鉴权/配额/5xx/provider 故障，commander 按适配器归一化 agent 名称与端点/模型组合（含 DeepSeek 双端点），任务未指定需求时回退到通用 agent，注入失败走恢复链路而不是重试死端点。
...
- **控制台：「Panda Paper」视觉重构与工作流打磨** —— 暖纸浅色主题与暖墨深色主题（拒绝纯黑与冷灰）、竹绿品牌色加深一档收沉稳、蓝色留给决策链路、衬线展示标题，会话与项目视图围绕工作流架构重做（291dc76）。

### 修复

- **TUI：终端输入栏转义序列残留修复** —— 因未捕获的鼠标移动或切片跨 read boundary 产生的不完整 ANSI 转义序列（如 SGR 鼠标报文碎片、光标位置报告、Kitty 扩展键、Alt-中括号等）现在被严格拦截过滤，彻底杜绝无效字符漏入输入框。
...
- **无配置文件时的路径解析** —— 路径解析不再回退到 `/etc` 而是用户目录，配置写入前先创建父目录；`node.card_path` 被用作卡片位置。
- **卡片编辑不再静默空操作** —— 添加已存在的 id、删除不存在的 id 在各前端都是错误；引擎在每次变更后热重载卡片，改动立即生效。
- **`ctrl+t` 之后滚轮仍然可用，任务执行期间也能滚** —— 切回选择模式时会重新请求 alternate scroll（Bubble Tea 自身从不发送 DECSET 1007，该标志必须手动跟随模式）；滚轮事件也被提前到模式闸门之前处理，回看长时间运行的任务不再只能靠 PageUp。任务执行期间的点击仍被忽略，避免误触中断。
...
- **队列任务重新可以跨设备路由（CLI 与看板）** —— 移除任务添加时对节点级工作目录的冗余绑定，使指定能力约束的任务能顺利派发到对端节点。
...
- **移除 `taskq_create` 管理工具** —— 任务创建现在完全由入口模型负责（输出 task JSON，由调度器立即执行），或来自看板/外部工具；队列工具（`taskq_list/show/cancel/priority/move`）保留，用于管理积压任务。依赖 `taskq_create` 的自动化脚本应改为输出任务或使用看板。
