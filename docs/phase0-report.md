...
**决策（2026-08-13）**：**接受 ~20MB，调整验收标准**（选项 1）。内存基线超标从「待决策」转为「已接受」，后续验收以 Micro 节点稳态 ≤30MB 为界，不再以 <8MB 为硬指标。
...
| Go 核心内存基线达标（<8MB）？ | ✅ 已接受超标 | ~19MB，调整标准为 ≤30MB，见 §4 |
