...
func (m tuiModel) liveRegion() string {
	...
	if ans := m.liveAnswer.String(); strings.TrimSpace(ans) != "" {
		parts = append(parts, wrap(ans, m.textWidth()))
...
// hintLine is the dim key legend under the input box.
func (m tuiModel) hintLine() string {
	...
	return m.th.muted.Render(strings.Join(hints, sep))
}
...
func (m tuiModel) welcome() string {
	...
	var sb strings.Builder
	sb.WriteString(m.th.heading.Render(i18n.T(m.loc, "repl.banner.title") + " v" + version))
	sb.WriteString("\n" + m.th.muted.Render(i18n.Tf(m.loc, "repl.banner.node", "node", m.r.cfg.Node.Name, "model", model)))
	sb.WriteString("\n" + m.th.muted.Render(i18n.Tf(m.loc, "repl.banner.dir", "dir", m.r.cfg.Storage.WorkPath)))
	sb.WriteString("\n" + m.th.muted.Render(i18n.T(m.loc, "tui.welcome.tips")))
	return m.th.welcome.Width(m.textWidth() + 2).Render(sb.String())
}
