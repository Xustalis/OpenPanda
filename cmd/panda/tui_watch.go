...
// watchMsg carries the report lines from one completed poll.
type watchMsg struct{ notes []string }
...
func watchTasks(r *repl) tea.Cmd {
	...
	return func() tea.Msg {
		...
		return watchMsg{notes: r.pollCompletions(context.Background())}
	}
}
...
// onWatch commits the poll's report lines and re-arms the watcher.
