...
func TestExecuteTool(t *testing.T) {
	...
	if got := executeTool(ctx, reg, &entry.ToolCall{Tool: "echo", Arguments: map[string]any{"x": 1}}, true, false, ""); got != "got 1" {
		t.Errorf("executeTool = %q, want success result", got)
	}
	// A tool failure is folded into the result, never a hard exit.
	if got := executeTool(ctx, reg, &entry.ToolCall{Tool: "failing"}, true, false, ""); !strings.Contains(got, "boom") {
		t.Errorf("executeTool failure = %q, want it to carry the error", got)
	}
	if got := executeTool(ctx, reg, &entry.ToolCall{Tool: "nope"}, true, false, ""); !strings.Contains(got, "未知工具") {
...
func TestExecuteToolTierGate(t *testing.T) {
	...
	if got := executeTool(ctx, reg, &entry.ToolCall{Tool: "echo"}, false, false, ""); !strings.Contains(got, "被拒") {
...
func TestExecuteTool_MultiLanguage(t *testing.T) {
	...
	resEn := executeTool(context.Background(), reg, &entry.ToolCall{Tool: "test_tool"}, false, false, "", i18n.English)
	...
	// Remembered denial names the standing decision instead of asking for a grant.
	resDenied := executeTool(context.Background(), reg, &entry.ToolCall{Tool: "test_tool"}, false, true, "", i18n.English)
	if !strings.Contains(resDenied, "previously denied") {
		t.Errorf("expected English remembered-deny refusal, got: %s", resDenied)
	}

	// Unknown tool in English
	unknownEn := executeTool(context.Background(), reg, &entry.ToolCall{Tool: "nonexistent"}, true, false, "", i18n.English)
	...
	unknownZh := executeTool(context.Background(), reg, &entry.ToolCall{Tool: "nonexistent"}, true, false, "", i18n.ChineseSimp)
